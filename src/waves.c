...
float magBuffer[NUM_SAMPLES*2];
q31_t correlation[NUM_SAMPLES*4-1];
...
//        arm_correlate_q31((const q31_t *)dstBufferA, NUM_SAMPLES, (const q31_t *)dstBufferA, NUM_SAMPLES, correlation);
        ...
        arm_cfft_q15(&arm_cfft_sR_q15_len2048, (q15_t *)dstBuffer, IFFTFLAG, BITREVERSE);

        for(i = 0; i < NUM_SAMPLES * 2; i++) {
            arm_status status = arm_sqrt_f32((int16_t)(dstBuffer[i]/65536) * (int16_t)(dstBuffer[i]/65536) + \
                                             (int16_t)(dstBuffer[i]%65536) * (int16_t)(dstBuffer[i]%65536), \
                                             &magBuffer[i]);
        }
